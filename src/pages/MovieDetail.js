...
const MovieDetail = () => {
  ...
  const GetCredits = `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${API_KEY}&language=en-US`
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => console.log(err));
  }, [movieDetailBaseUrl, videoUrl, GetCredits]);
          ...
          <div className="col-md-8 d-flex flex-column ">
            ...
            <div className="card-body">
              ...
              {(credits?.cast?.map(( { name, id } ) => {
                  return(
                    <p key={id} className="card-text">{name}</p>
                  )
              }))}
            </div>
